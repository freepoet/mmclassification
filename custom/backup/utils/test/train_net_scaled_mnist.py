...
class LeNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(  # input_size=(1*28*28)
            nn.Conv2d(1, 6, 5, 1, 2),  # padding=2保证输入输出尺寸相同   size=32 after padding
...
if __name__ == '__main__':
    ...
    train_loader = torch.utils.data.DataLoader(  # 加载训练数据
        datasets.Scaled_MNIST('/media/n/SanDiskSSD/HardDisk/data',
                       train=True,
                       download=False,
    ...
    test_loader = torch.utils.data.DataLoader(  # 加载训练数据，详细用法参考我的Pytorch打怪路（一）系列-（1）
        datasets.Scaled_MNIST('/media/n/SanDiskSSD/HardDisk/data', train=False, transform=transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.1307,), (0.3081,))  # 数据集给出的均值和标准差系数，每个数据集都不同的，都数据集提供方给出的
        ])),
